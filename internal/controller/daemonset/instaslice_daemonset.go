...
func (r *InstaSliceDaemonsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for podUID, allocResult := range instaslice.Status.PodAllocationResults {
		...
		if allocResult.AllocationStatus.AllocationStatusController == inferencev1alpha1.AllocationStatusDeleting &&
			...
			// Store it back into .status.PodAllocationResults
			instaslice.Status.PodAllocationResults[podUID] = newAlloc
			if err := r.Status().Update(ctx, &instaslice); err != nil {
				log.Error(err, "error updating Instaslice status for pod cleanup")
				return ctrl.Result{Requeue: true}, nil
			}
			return ctrl.Result{}, nil
		}
		...
		if allocResult.AllocationStatus.AllocationStatusController == inferencev1alpha1.AllocationStatusCreating &&
			...
			// newAlloc := allocResult
			// newAlloc.AllocationStatus = inferencev1alpha1.AllocationStatusCreated
			// newCondition := metav1.Condition{
			// 	Type:               string(allocResult.AllocationStatus),
			// 	Status:             metav1.ConditionTrue,
			// 	Reason:             "AllocationStatusUpdate",
			...
			// if err := r.Status().Update(ctx, instaslice); err != nil {
			// 	log.Error(err, "error updating Instaslice status after MIG creation")
			// 	return ctrl.Result{Requeue: true}, nil
			// }
...
func (r *InstaSliceDaemonsetReconciler) cleanUpCiAndGi(ctx context.Context, allocationResult *inferencev1alpha1.AllocationResult) error {
	log := logr.FromContext(ctx)
	...
	for _, migdevice := range migInfos {
		if migdevice.uuid == allocationResult.GPUUUID && migdevice.start == allocationResult.MigPlacement.Start {
			...
			log.Info("Successfully destroyed MIG resources", "podRef", "allocationResult.PodRef.Name") // TODO - FixMe
			return nil
		}
	}

	exists, err := r.checkConfigMapExists(ctx, string(allocationResult.ConfigMapResourceIdentifier), "PodRef.Name")
	if err != nil {
		return err
	}
	if exists {
		...
		return fmt.Errorf("MIG slice not found for GPUUUID %v and Start %v", allocationResult.GPUUUID, allocationResult.MigPlacement.Start)
	}
	return nil
}
...
func (r *InstaSliceDaemonsetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	mgrAddErr := mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		...
		if r.Config.EmulatorModeEnable {
			totalEmulatedGPUMemory, err := CalculateTotalMemoryGB(instaslice.Status.NodeResources.NodeGPUs)
			if err != nil {
...
func (r *InstaSliceDaemonsetReconciler) discoverMigEnabledGpuWithSlices() ([]string, error) {
	...
	// instaslice.Status.NodeResources.NodeGPUs
	// instaslice.Status.Processed = true // TODO - fix with condition readiness.
	...
	instaslice, _, failed, err := r.discoverAvailableProfilesOnGpus(instaslice)
	if failed || err != nil {
		return nil, err
	}
