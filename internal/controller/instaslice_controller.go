...
func (r *InstasliceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if pod.Status.Phase == v1.PodFailed && controllerutil.ContainsFinalizer(pod, FinalizerName) {
		for _, instaslice := range instasliceList.Items {
			for uuid, allocation := range instaslice.Status.PodAllocationResults {
				if pod.UID == uuid {
					if allocation.AllocationStatus.AllocationStatusController == inferencev1alpha1.AllocationStatusCreating {
					...
					if allocation.AllocationStatus.AllocationStatusDaemonset == inferencev1alpha1.AllocationStatusCreated || allocation.AllocationStatus.AllocationStatusController == inferencev1alpha1.AllocationStatusUngated {
						...
						resultDeleting, err := r.setInstasliceAllocationToDeleting(ctx, instaslice.Name, string(pod.UID), &allocation, &allocRequest)
						if err != nil {
			...
			for uuid, allocation := range instaslice.Status.PodAllocationResults {
				if uuid == pod.UID {
					if allocation.AllocationStatus.AllocationStatusDaemonset != inferencev1alpha1.AllocationStatusDeleted {
						...
						result, err := r.setInstasliceAllocationToDeleting(ctx, instaslice.Name, string(pod.UID), &allocation, &allocRequest)
						if err != nil {
							return result, err
						}
						// return and rely on daemonset to se allocation status to created
	...
	if isPodGated {
		...
		for _, instaslice := range instasliceList.Items {
			for uuid, _ := range instaslice.Spec.PodAllocationRequests {
		...
		if !podHasNodeAllocation {
			...
			for _, instaslice := range instasliceList.Items {
				...
				if err != nil {
					log.Info("FFFFFFFFFFFFFFFFFFFFFFF ", "err", err)
					continue
				}
				podHasNodeAllocation = true
				if podHasNodeAllocation {
...
func (r *InstasliceReconciler) setInstasliceAllocationToDeleting(ctx context.Context, instasliceName string, podUUID string, allocResult *inferencev1alpha1.AllocationResult, allocRequest *inferencev1alpha1.AllocationRequest) (ctrl.Result, error) {
	log := logr.FromContext(ctx)
